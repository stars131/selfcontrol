...
- 推荐 Node.js 主版本：`20`
- 仓库已提供 `.python-version` 与 `.nvmrc` 作为工具链基线
...
在启动容器前，建议先跑一次环境体检脚本，确认 Node、Python、Docker、Git hooks 和依赖状态：
...
如果希望在 `git push` 前自动执行项目级校验，可以安装仓库级 Git hooks：
...
如果 hooks 已安装，`git push` 时会自动执行项目级校验。确需临时跳过时，可设置 `SELFCONTROL_SKIP_VERIFY=1`，但应只用于例外情况。

## 6. 当前建议

- 本地 Windows + Linux VM 开发时，默认保持 `MEDIA_PROCESSING_MODE=async`
- 仅在临时调试 API 或最小化依赖时切回 `sync`
- 服务器部署时应同时启动 `api`、`worker`、`redis`，否则异步媒体任务不会被消费
- 生产环境必须设置强 `SECRET_KEY`，并关闭 `AUTO_CREATE_TABLES`，改为显式执行 Alembic 迁移
- 排查环境问题前，优先运行 `doctor-dev`，再决定是否需要重新 bootstrap 或重启容器
