...
const AUDIT_LOG_ACTION_LABELS: Record<LocaleCode, Record<string, string>> = {
  ...
  "zh-CN": {
    ...
    "share_link.accept": "接受分享链接",
    "share_link.create": "创建分享链接",
    "share_link.update": "更新分享链接",
  ...
  ja: {
    "knowledge.reindex": "ナレッジインデックスを再構築",
    "media.delete": "メディアを削除",
    "media.dead_letter_bulk_retry": "デッドレターメディアを一括再試行",
    "media.retention_archive": "保留メディアをアーカイブ",
    "media.retention_cleanup": "保留メディアをクリーンアップ",
    "media.retry_processing": "メディア処理を再試行",
    "media.upload": "メディアをアップロード",
    "provider_config.update": "プロバイダー設定を更新",
    "record.create": "記録を作成",
