...
const ISSUE_CATEGORY_LABELS: Record<LocaleCode, Record<string, string>> = {
  "zh-CN": {
    ...
    provider_disabled: "提供方未启用",
    missing_secret: "缺少密钥",
    provider_config_error: "提供方配置异常",
    provider_unsupported: "当前路径不支持该提供方",
    remote_asset_missing: "远端文件不存在",
    transient_remote_failure: "远端临时故障",
    provider_not_ready: "提供方尚未就绪",
...
const ACTION_LABELS: Record<LocaleCode, Record<string, string>> = {
  "zh-CN": {
    ...
    enable_provider: "启用对应的提取提供方",
    configure_secret: "配置提供方密钥",
    fix_provider_config: "修正提供方设置",
    switch_provider: "切换到受支持的提供方",
    review_remote_asset: "检查远端文件或清理失效引用",
    retry_after_remote_check: "检查远端服务后重试",
    retry_when_ready: "等待提供方完成后重试",
    increase_retry_budget_or_retry_now: "调整重试策略或立即重试",
    enable_auto_retry_or_retry_now: "启用自动重试或立即重试",
    manual_retry: "人工确认后重试",
    check_remote_storage_health: "检查远端存储健康状态",
    review_and_retry: "查看错误详情后重试",
  },
  en: {
...
const ACTION_DETAIL_LABELS: Record<LocaleCode, Record<string, string>> = {
  "zh-CN": {
    ...
    enable_provider: "先在工作区中启用 OCR、ASR 或视频转写提供方，再进行重试。",
    configure_secret: "先在服务端环境变量中配置所需密钥，再重新处理。",
    fix_provider_config: "请先检查当前工作区的媒体存储端点和提供方配置。",
    switch_provider: "当前媒体类型或传输模式暂不支持这个提供方流程。",
    review_remote_asset: "远端对象可能已被外部删除，需要确认或移除失效引用。",
    retry_after_remote_check: "远端服务当前可能限流或暂时不可用。",
    ...
    review_and_retry: "建议先查看提供方设置和错误详情，再决定是否重试。",
  },
  en: {
  ...
  ja: {
    ...
    manual_retry: "この問題は自動再試行に不向きと判断され、運用者の確認が必要です。",
    check_remote_storage_health: "リモート媒体の取得中にサーバーで失敗しました。",
