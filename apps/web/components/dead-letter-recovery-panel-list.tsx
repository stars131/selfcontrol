...
export function DeadLetterRecoveryPanelList(props: DeadLetterRecoveryPanelListProps) {
  return <div className="record-list compact-list">{props.mediaDeadLetterOverview.items.map((item) => <DeadLetterRecoveryItemCard key={item.media_id} {...buildDeadLetterRecoveryItemCardProps({ ...props, item })} />)}</div>;
}
