...
import { getProcessingStatusLabel, getRemoteFetchStatusLabel, getRetryStateLabel } from "../lib/media-issue-display";
...
export function MediaAssetCardMetadataTags({ asset, mediaIssueCopy }: MediaAssetCardMetadataTagsProps) {
  ...
  return <div className="tag-row"><span className="tag">{getProcessingStatusLabel(locale, asset.processing_status)}</span><span className="tag">{getStorageProviderLabel(locale, asset.storage_provider)}</span><span className="tag">{formatMediaSize(asset)}</span>{processingSource ? <span className="tag">{processingSource}</span> : null}{extractionMode ? <span className="tag">{extractionMode}</span> : null}{remoteFetchStatus ? <span className="tag">{mediaIssueCopy.fetchPrefix} {getRemoteFetchStatusLabel(locale, remoteFetchStatus)}</span> : null}{retryState && retryState !== "idle" ? <span className="tag">{mediaIssueCopy.retryStatePrefix} {getRetryStateLabel(locale, retryState)}</span> : null}{retryCount !== null ? <span className="tag">{mediaIssueCopy.retries} {retryCount}{retryMaxAttempts !== null ? `/${retryMaxAttempts}` : ""}</span> : null}{typeof asset.metadata_json.file_extension === "string" && asset.metadata_json.file_extension ? <span className="tag">{String(asset.metadata_json.file_extension)}</span> : null}</div>;
}
