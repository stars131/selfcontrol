...
export function MediaAssetCardMetadataTags({ asset, mediaIssueCopy }: MediaAssetCardMetadataTagsProps) {
  ...
  return <div className="tag-row"><span className="tag">{getProcessingStatusLabel(locale, asset.processing_status)}</span><span className="tag">{getStorageProviderLabel(locale, asset.storage_provider)}</span><span className="tag">{formatMediaSize(asset)}</span><MediaAssetCardProcessingSourceTag processingSource={processingSource} /><MediaAssetCardExtractionModeTag extractionMode={extractionMode} /><MediaAssetCardRemoteFetchTag locale={locale} mediaIssueCopy={mediaIssueCopy} remoteFetchStatus={remoteFetchStatus} /><MediaAssetCardRetryStateTag locale={locale} mediaIssueCopy={mediaIssueCopy} retryState={retryState} /><MediaAssetCardRetryCountTag mediaIssueCopy={mediaIssueCopy} retryCount={retryCount} retryMaxAttempts={retryMaxAttempts} /><MediaAssetCardFileExtensionTag asset={asset} /></div>;
}
