...
export function RecentMediaIssuesPanel({ mediaIssueCopy, ...props }: RecentMediaIssuesPanelProps) {
  return <div className="record-card form-stack" style={{ marginBottom: 16 }}><RecentMediaIssuesPanelIntro mediaIssueCopy={mediaIssueCopy} />{props.mediaProcessingOverview.recent_issues.length ? <RecentMediaIssuesPanelList {...buildRecentMediaIssuesPanelListProps({ mediaIssueCopy, ...props })} /> : <RecentMediaIssuesPanelEmpty mediaIssueCopy={mediaIssueCopy} />}</div>;
}
