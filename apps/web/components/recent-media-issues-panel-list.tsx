...
export function RecentMediaIssuesPanelList(props: RecentMediaIssuesPanelListProps) {
  return <div className="record-list compact-list" style={{ marginTop: 16 }}>{props.mediaProcessingOverview.recent_issues.map((issue) => <RecentMediaIssueCard key={issue.media_id} {...buildRecentMediaIssueCardProps({ ...props, issue })} />)}</div>;
}
